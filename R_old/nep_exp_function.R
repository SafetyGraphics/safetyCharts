...
lb1 <- nep_explorer_data %>%
    filter(TEST %in% c("Creatinine", "Cystatin C", "eGFR", "eGFRcys")) %>%
...
lb4 <- lb3 %>%
    mutate(
        CHG = ifelse(!is.na(TRTSDT) & !is.na(ADT) & ADT >= TRTSDT & is.na(ABLFL) & !is.na(AVAL) & !is.na(BASE), AVAL - BASE, NA),
    ...
    arrange(USUBJID, PARCAT, PARAM, VISITNUM) %>%
    select(-c(BASE, LBDTC_, LBDTC1, LBDTC2, LBDTC3, BASEDT, TRTSDTC))



################### create variables for each test for CHG,PCHG and FCHG##
lb5_creat <- lb4 %>%
    filter(PARAMCD == "creat") %>%
    mutate(CREAT_CHG = CHG, CREAT_PCHG = PCHG, CREAT_FCHG = FCHG) %>%
    select(USUBJID, VISITNUM, PARCAT, ADT, CREAT_CHG, CREAT_PCHG, CREAT_FCHG)

lb5_cystac <- lb4 %>%
    filter(PARAMCD == "cystac") %>%
    mutate(CYSTAC_CHG = CHG, CYSTAC_PCHG = PCHG, CYSTAC_FCHG = FCHG) %>%
    select(USUBJID, VISITNUM, PARCAT, ADT, CYSTAC_CHG, CYSTAC_PCHG, CYSTAC_FCHG)
lb5_egfr_creat <- lb4 %>%
    filter(PARAMCD == "eGFR_creat") %>%
    mutate(eGFR_CREAT_CHG = CHG, eGFR_CREAT_PCHG = PCHG, eGFR_CREAT_FCHG = FCHG) %>%
    select(USUBJID, VISITNUM, PARCAT, ADT, eGFR_CREAT_CHG, eGFR_CREAT_PCHG, eGFR_CREAT_FCHG)
lb5_egfr_cystac <- lb4 %>%
    filter(PARAMCD == "eGFR_cystac") %>%
    mutate(eGFR_CYSTAC_CHG = CHG, eGFR_CYSTAC_PCHG = PCHG, eGFR_CYSTAC_FCHG = FCHG) %>%
...
lb4a <- lb4 %>%
    select(-c(TEST)) %>%
    distinct(USUBJID, VISITNUM, PARCAT, ADT, .keep_all = TRUE)
...
###################################################################
lb7 <- lb4 %>%
    filter(PARAMCD %in% c("creat", "cystac", "eGFR_creat", "eGFR_cystac")) %>%
    arrange(USUBJID, PARCAT, PARAM, VISITNUM, ADY)
